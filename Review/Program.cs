...
Console.WriteLine(count);   //expect to see 11
...
//The built-in Math library works mosrtly with doubles
...
//That is unique to C#. Earlier languages you put the brackets after variable name

Console.WriteLine();
Console.WriteLine("Arrray tests");
...
//so the first element changes from 10 to 11, second 20 to 21.
...
//Strings are technically objects stores in shared memory, but C#
// does a godo job of hiding that, so we don;t really need to know that.
...
Console.WriteLine($"The word fox occures at position {pos}");
...
spelling.Add("One Hundred", 100); //Same effect but we're adding the mapping as a single pair.
...
// Tehcnically theyre stored as numbers, starting with 0.
...
class Rectangle
{
    ...
    //To add a ToString, start typing public override and click ToString
    // and then remove the code it auo included
